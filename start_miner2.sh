...
if [[ $# -lt 1 || -z "${1:-}" ]]; then
  ...
  echo "MODEL (opcjonalnie): gen7heur7"
  exit 1
fi
...
case "$MODEL" in
  ...
  "gen7heur7" )
    ;;
  * )
    echo "ERROR: Niepoprawny MODEL='$MODEL'. Dozwolone: gen7heur7"
    exit 1
    ;;
esac
